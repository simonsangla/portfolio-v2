...
- **2026-04-20 — Waves 2+3 (Stages A–D, [#22](https://github.com/simonsangla/portfolio-v2/pull/22)–[#25](https://github.com/simonsangla/portfolio-v2/pull/25), main at `c3a296d`).** Stage A (T-4, `90d083f`): `NEXT_PUBLIC_TALLY_URL`-driven `site.intakeUrl`; mailto removed from intake surfaces; `intake_open` fires with `source` label per CTA. Stage B (T-5, `3d235d7`): `/faq` with 8 MDX entries (≥80 words each), plain-string search + tag filter via `useSearchParams`; `faq_search` / `faq_tag_filter` / `faq_ask_cta_click` events. Stage C (T-6, `e76139f`): `/case-studies` index + `/case-studies/[slug]` detail; seed `content/case-studies/tripadvisor-precursor.mdx` (889 body words, 8 sections — Problem / Approach / Stack / Metrics before / Metrics after / What I built / What I'd do differently / Lessons); landing proof callout under proofs carousel; MetricPilot proof cross-link (TripAdvisor count stays at 1); `case_study_view` on mount + `case_study_cta_click` on bottom CTA. Stage D (T-7, `c3a296d`): `/services`, `/training`, `/products`, `/cv` new routes; landing slimmed 550L/1903W → 207L/625W (−62.4% / −67.2%); `Nav.tsx` restructured as client component (6 links Services · Training · Products · Fractional · FAQ · CV, name logo → `/cv`, `nav_click` fires on all 7 surfaces); sitemap entries /services /training /products @ 0.9 + /cv @ 0.7; per-route unique `<title>` + `<meta description>`; Lighthouse desktop ≥90 on all 4 new routes (98/100/100/100). Redirects = none (zero external-share evidence for anchors). All 4 PRs: lint ✅ · typecheck ✅ · build ✅ · Vercel ✅ · human preview gate passed on first review.
